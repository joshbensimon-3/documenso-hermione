...
export const DocumentSigningFieldsUninserted = ({ children }: { children: React.ReactNode }) => {
  return (
    <p className="overflow-visible whitespace-normal break-words text-[clamp(0.425rem,12cqw,0.825rem)] text-foreground duration-200 group-hover:text-recipient-green">
      {children}
    </p>
  );
};
...
export const DocumentSigningFieldsInserted = ({ children, textAlign = 'left' }: DocumentSigningFieldsInsertedProps) => {
  return (
    <div className="flex h-full w-full items-center overflow-visible">
      <p
        className={cn(
          'w-full max-w-full whitespace-normal break-words text-left text-[clamp(0.425rem,12cqw,0.825rem)] text-foreground duration-200',
          {
